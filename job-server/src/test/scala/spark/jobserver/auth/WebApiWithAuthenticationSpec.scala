...
class WebApiWithAuthenticationSpec extends FunSpec with Matchers
  ...
  private def routesWithTimeout(useAsProxyUser: Boolean, authTimeout: Int): Route = {
    val testConfig = config.withValue("shiro.authentication-timeout",
      ConfigValueFactory.fromAnyRef(authTimeout)).withValue("shiro.use-as-proxy-user",
      ConfigValueFactory.fromAnyRef(useAsProxyUser))
    val api = new WebApi(system, testConfig, dummyPort, dummyActor, dummyActor, dummyActor, dummyActor) {
      private def asShiroAuthenticatorWithWait(authTimeout: Int)(implicit ec: ExecutionContext):
      AuthMagnet[AuthInfo] = {
  ...
  private val addedContexts = new mutable.HashSet[String] with mutable.SynchronizedSet[String]
  ...
  // These tests work maybe like...1 out of every 10 runs?
