...
object DAOMetrics extends YammerMetrics {
  ...
  val resultRead = timer("result-read-duration", TimeUnit.MILLISECONDS)
  val resultWrite = timer("result-write-duration", TimeUnit.MILLISECONDS)
  val resultDelete = timer("result-delete-duration", TimeUnit.MILLISECONDS)
