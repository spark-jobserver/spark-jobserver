...
class CornerCasesTests extends FreeSpec with Matchers with BeforeAndAfterAllConfigMap {
  ...
  "DELETE /binaries should not delete binaries with running jobs" in {
    ...
    val response1 = sttp.post(uri"$SJS/binaries/$appName")
        .body(byteArray)
    ...
    val response2 = sttp.post(uri"$SJS/jobs?appName=$appName&classPath=spark.jobserver.LongPiJob")
        .body("stress.test.longpijob.duration = 30")
        .send()
    ...
    val response3 = sttp.delete(uri"$SJS/binaries/$appName")
        .send()
