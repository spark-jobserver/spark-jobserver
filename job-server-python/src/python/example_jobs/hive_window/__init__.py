...
class HiveWindowSessionJob(SparkJob):

    def validate(self, session, runtime, config):
        problems = []
        job_data = None
        if config.get('input.data', None):
            job_data = config.get('input.data')
    ...
    def run_job(self, session, runtime, data):
        rdd = session.sparkContext.parallelize(data)
        df = session.createDataFrame(rdd, ['name', 'age', 'salary'])
        df.registerTempTable('people')
        # Window functions only available on
        # HiveContext so differentiates from a SQLContext job
        query = session.sql("""
